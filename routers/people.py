...
"""
People Suggest & Network & Search API (Azure MySQL)
...
- /people/search : 自然文の単一クエリ → LIKEベースの人物検索（business_cards + companies）
"""
...
# --- Search 用（フロントの期待に合わせて candidates 配列を返す） ---
class PeopleSearchReq(BaseModel):
    query: str = Field(..., description="自然文の検索条件（例：'富山でECに強い人'）")
...
# ========== 追加: 自然文クエリで人物検索 ==========
@router.post("/search")
def people_search(body: PeopleSearchReq) -> Dict[str, Any]:
    """
    単一の自然文クエリから人物を検索して返す。
    - business_cards と companies を結合し、主要カラムへ LIKE
    - トークンごとに AND（すべて含む）で絞り込み
    - スコアは結合文字列へのトークン一致数（簡易）
    レスポンスはフロントの期待に合わせ {"candidates":[{id,name,department,title,company,score,...}]} 形式。
    """
    ...
    with get_conn() as conn:
        ...
        # 検索対象カラムを結合（NULLも安全に）
        concat_expr = "CONCAT_WS(' ', COALESCE(bc.company, c.name), bc.name, bc.department, bc.position, bc.memo, c.location)"

        # すべてのトークンを含む（AND）
        where_clause = " AND ".join([f"{concat_expr} LIKE %s" for _ in tokens])

        # スコア：一致トークン数
        score_expr = " + ".join([f"(CASE WHEN {concat_expr} LIKE %s THEN 1 ELSE 0 END)" for _ in tokens])
        ...
        sql = f"""
          ...
          LEFT JOIN companies c ON c.id = bc.company_id
          WHERE {where_clause}
          ORDER BY score DESC, bc.id DESC
          LIMIT %s
        """

        # パラメータ：WHERE 用に tokens、続けて SCORE 用に tokens、最後に LIMIT
        where_params = [f"%{t}%" for t in tokens]
        score_params = [f"%{t}%" for t in tokens]
        params = where_params + score_params + [body.top_k]

        cur.execute(sql, params)
        rows = cur.fetchall()

    # フロントの Candidate 型に合わせて整形（id, department, title など）
    candidates = [{
        "id":         r["card_id"],     # ← card_id を id に変換
        "name":       r["name"],
        "company":    r.get("company") or "",
        "department": r.get("department"),
        "title":      r.get("position"),
        "skills":     None,             # 手元のスキーマに無いため None
        "avatar_url": None,             # 手元のスキーマに無いため None
